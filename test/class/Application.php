    ...
    public function actionFormSubmit($data) {
		...
		if($phone=='') //проверка телефона на наличие значение
		   ...
		   /* if(!preg_match('/^\+380(\d{2}\)d{3}-\d{2}-\d{2}$/',$phone))
		        $errors['phone']='введите валидный номер'; */
		...
		if(!$comment==''){
		if(strlen($comment)<=1024)//проверка комментария на количество символов
		    $errors['comment']='количество символов должно быть больше 1024';
		if(preg_match("/<[^<]+>/",$comment))//проверка комментария на наличие тегов
		    $errors['comment']='теги не допустимы';
		}
